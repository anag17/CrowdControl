...
namespace RogoDigital.Lipsync {
	public class MarkerSettingsWindow : ModalWindow {
		...
		void OnGUI () {
			...
			if (markerType == 0) {
				if (pMarker.Length == 1) {
					GUILayout.Label("Editing " + setup.settings.phonemeSet.phonemes[pMarker[0].phonemeNumber].name + " Phoneme Marker at " + (pMarker[0].time * setup.fileLength).ToString() + "s.");
				} else {
			...
			if (markerType == 0) {
				...
				phonemeNumber = FlaggedIntPopup("Phoneme", phonemeNumber, phonemeNames, phonemeValues, ref setPhonemeNumber);
			} else {
			...
			if (GUILayout.Button("Accept", GUILayout.MinWidth(100), GUILayout.Height(20))) {
				if (markerType == 0) {
					for (int i = 0; i < pMarker.Length; i++) {
						if (setTime) pMarker[i].time = time;
						if (setPhonemeNumber) pMarker[i].phonemeNumber = phonemeNumber;
						if (setIntensity) pMarker[i].intensity = intensity;

						if (setModifierOn) pMarker[i].useRandomness = modifierOn;
						if (setIntensityModifier) pMarker[i].intensityRandomness = intensityModifier;
						if (setBlendableModifier) pMarker[i].blendableRandomness = blendableModifier;
						if (setBonePositionModifier) pMarker[i].bonePositionRandomness = bonePositionModifier;
						if (setBoneRotationModifier) pMarker[i].boneRotationRandomness = boneRotationModifier;
					}
				} else {
					for (int i = 0; i < eMarker.Length; i++) {
						if (setStartTime) eMarker[i].startTime = startTime;
						if (setEndTime) eMarker[i].endTime = endTime;
						if (setIntensity) eMarker[i].intensity = intensity;

						if (setModifierOn) eMarker[i].continuousVariation = modifierOn;
						if (setMaxVariationFrequency) eMarker[i].variationFrequency = maxVariationFrequency;
						if (setIntensityModifier) eMarker[i].intensityVariation = intensityModifier;
						if (setBlendableModifier) eMarker[i].blendableVariation = blendableModifier;
						if (setBonePositionModifier) eMarker[i].bonePositionVariation = bonePositionModifier;
						if (setBoneRotationModifier) eMarker[i].boneRotationVariation = boneRotationModifier;
					}
				}
				setup.changed = true;
